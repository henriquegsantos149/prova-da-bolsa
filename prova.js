document.addEventListener('DOMContentLoaded', () => {
    ...
    function calculateScoreAndRedirect() {
        ...
        if (acertos >= 11) {
            desconto = 70; // 11 a 12 acertos
        } else if (acertos === 10) {
            desconto = 65; // 10 acertos
        } else if (acertos === 9) {
            desconto = 60; // 9 acertos
        } else if (acertos === 8) {
            desconto = 55; // 8 acertos
        } else if (acertos >= 5) {
            desconto = 50; // 5 a 7 acertos
        } else if (acertos === 4) {
            desconto = 30; // 4 acertos
        } else {
            desconto = 0; // 0 a 3 acertos
        }
        ...
        fetch('https://node2.rodrigogreco.com.br/webhook/prova/bolsa/sim', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
        }).catch(err => console.error("Erro ao enviar webhook de prova realizada:", err));
